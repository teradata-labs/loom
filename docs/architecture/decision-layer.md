...
**Deciders can be well ordered and badly scaled.** Measured against labels on 1,536 recall candidates, Jev's probabilities rise monotonically from 3% evidence below 0.1 to 99% above 0.9, but a 0.15 from it means 62% in reality, not 15%. A site's `true_min` therefore belongs on the decider's own scale, taken from a calibration sweep against labels, not set to 0.5 because that is where a calibrated model would put it. For Jev on graph-memory recall the sweep says 0.1; see `docs/research/decision-layer-phase3-report.md` §9.

**`act_min` is not a keep threshold.** A band has two numbers and they answer different questions. `act_min` is about *decisiveness*: how far from 0.5 an answer must be before the site may act on it at all (`AnswerConfidence`, which for a Noul is `|2p - 1|`). `true_min` is about *which side of the line* a decisive answer falls on: the probability at or above which a Noul counts as true, which is the keep threshold at a rerank, "valid" at a stage gate and "consensus reached" at a debate. It defaults to 0.5 (`decision.DefaultTrueMin`) and is read through `Band.IsTrue`, never as a raw field.

Lowering the two has opposite effects, which is worth stating because getting it backwards is easy: lowering `true_min` keeps more, while lowering `act_min` keeps **fewer**, because candidates just under 0.5 stop being uncertain (and uncertain candidates are always kept) and become decisive enough to drop. The LongMemEval rerun of 2026-09-24 made exactly that mistake and measured a 9% keep rate where 18% was intended.
...
|---|---|---|---|
| `memory.extract` (`Agent.extractGraphMemoryAsync`) ✅ code | one `Noul`: does this conversation window hold a durable fact worth keeping? State is the window rendered as role and text, tool payloads dropped (≤6,000 runes). **One-sided**: a confident *no* skips the extraction call; nothing else changes anything. Uncertain, no answer, decider error and a confident yes all extract exactly as before | the answer clears the band and says nothing durable is here (`sites.ExtractVerdict`) | the extraction call itself, unchanged |
| `skill.route` (`skills/index.Router.Route`) ✅ code | one `Noul` per option at each node visited: the child subtrees to descend into and the skills to select, asked together because the walk decides both at once. State is the message (≤1,500 runes), the node's title and summary, and each option's title and text. A subtree is descended on a relevant answer **or** an uncertain one, because pruning loses everything beneath it; a skill is selected only on a confident yes, because a wrong one costs prompt budget on every later turn. Skills are ordered by probability and capped at the router's `maxCandidates` | at least one option cleared the band **and** something was chosen (`sites.SkillRouteContributed`). Discarding every option at a node is the answer most worth a second opinion, so it falls through rather than pruning the whole tree on one model's say-so | the tree-walk LLM (`askDecision`) or the fat-leaf pick (`pickFromFatLeaf`), whichever this node would have used; reference = what it chose |
| `recall.rerank` (`Agent.rerankMemories`) ✅ code | keep a candidate when its Noul says relevant (p ≥ the band's `true_min`, default 0.5) **or** the answer does not clear the band (an uncertain memory is kept, never dropped) | at least one answer cleared the band (`sites.RerankContributed`); if every answer is uncertain the LLM rerank runs | `rerankMemoriesLLM` |
...
**`memory.extract` is the one site whose reference is observed truth.** Every other site in this table compares the decider against another mechanism's opinion, so agreement is not accuracy. Here the shadow row is labelled by what extraction actually did: the run stores memories or it does not, and that yes/no is recorded as the reference. Shadow mode therefore produces a labelled dataset for free, and the acceptance question is answerable without hand-labelling anything.

The polarity is chosen for the failure that matters. A wrong "nothing here" loses a memory permanently, because nothing revisits that window; a wrong "something here" costs one extraction that finds nothing, which is what happens on every fire today. So the gate may only make the cheap mistake. That also suits a decider readier to say yes than no — the measured shape at `skill.route` was recall 96.7% at precision 50.9% — because the answer it is reluctant to give is the only one that can lose data.

**Measured, and not recommended for a live band.** Against real Jev on a 73-message labelled corpus (32 skills, 8 domains, plus 17 out-of-scope messages), swept across ten band settings, the best configuration routed 16 of 73 correctly — 22% — with 33 wrong-domain loads and 42 of 73 messages routing nowhere. The per-decision view explains the shape: over 350 labelled option judgements, recall was 96.7% and precision 50.9%. Jev almost never rejects the right option and says yes to roughly twice as many options as it should. In a fan-out rerank that is survivable, because keeping extra candidates is cheap; in a tree walk it means descending into most subtrees and then selecting the wrong skill out of the ones it reaches. `skill.route` therefore stays in shadow. The evaluation lives in `TestRouteCorpusAgainstJev` and fails on purpose until a configuration clears the floor; it is skipped without credentials so CI never runs it. Caveats: the corpus is hand-authored, and this is one prompt formulation, so it bounds this phrasing rather than the model in general.

**Why this one is different.** `skill.route` removes the most calls of any site: the walk makes one generative call per node visited, sequentially, before the agent's turn can start, plus another at any leaf holding more skills than the candidate cap. A live band replaces each with one typed request. The walk already degrades to FTS5 when it cannot decide, so the fallback path was there before the decision layer was.

**Status per site.** ⚠️ `skill.route`: live path implemented and measured against a real decider; the measurement says keep it in shadow (see above). ⚠️ `recall.rerank`, `tool_search.rerank`, `workflow.branch`, `stage.validation`, `swarm.tie_break`, `debate.consensus`: live path implemented and tested with the mock decider; no shadow report has been produced for any of them against a real decider, so no band is recommended yet. 📋 `OutputPolicy.acceptance_criteria` (Noul per criterion): the field exists on the proto but nothing in the orchestrator evaluates it today, and giving it a decider is a feature with its own design (who supplies the criteria, which agent's layer), not a drop-in; left for a later phase.
...
**Latency: fast in the middle, terrible in the tail.** A second run reproduced the result (97.4% at the same cut) and measured the distribution rather than the mean: p50 485 ms, p90 2.3 s, p99 52 s, against a p50 of 1.0 s for the gpt-5.2 judge timed on the same rows. So at the median the typed judge is about twice as fast, and the 3.2 s average is an artefact of a tail spent in the client's retry ladder. Every one of the 15 failures in that run was a gateway HTTP 503 or 429, none a bad answer. On a reliable endpoint this is faster and cheaper; on the current gateway the mean is not worth quoting and the tail is the thing to fix.
...
**The throttle admits one whole fan-out.** The Jev client's token bucket held one second's worth of tokens, which was right when a decision was one request and wrong once chunking made it several: at 80 requests per minute a 64-candidate rerank queued its own four chunks behind 1.33 tokens and missed the 4 s budget it was given. The burst now floors at `decision.DefaultChunkConcurrency`, so a single decision never queues against itself; the average rate is unchanged, only the smoothing. Waiting for a token also respects the caller's deadline: when the next token is further away than the deadline allows, the client returns `ErrRateLimited` at once instead of consuming the budget and reporting a deadline error the caller cannot tell from a hung provider. And a throttle is never a reason to bisect, since splitting doubles the request count against the budget that just refused one.
