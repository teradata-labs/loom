...
The conditional and pipeline executors borrow the condition or stage agent's decision layer through `Agent.LiveDecide`, `Agent.RecordDecisionAsync` and `Agent.RunDecisionShadow`; bands for `workflow.branch` and `stage.validation` are therefore configured on that agent's YAML. Rows are keyed to the condition or validation session id. `stage.validation` is the first site whose code honours `DecisionBandMode`.

**Status per site.** ⚠️ `recall.rerank`, `tool_search.rerank`, `workflow.branch`, `stage.validation`: live path implemented and tested with the mock decider; no shadow report has been produced for any of them against a real decider, so no band is recommended yet. 📋 swarm/debate: not started. 📋 `OutputPolicy.acceptance_criteria` (Noul per criterion): the field exists on the proto but nothing in the orchestrator evaluates it today, and giving it a decider is a feature with its own design (who supplies the criteria, which agent's layer), not a drop-in; left for a later phase.
