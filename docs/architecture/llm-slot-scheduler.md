...
**Status**: 📋 Planned (design proposal — nothing in this document is implemented unless explicitly marked otherwise)
**Related**: issues #346, #348, #349; PRs #347 (keyed shared limiters), #350 (limiter on every construction path, concurrent dispatch, YAML `rate_limit` loading, retryable 429s)
...
| Class | Who | Why |
|---|---|---|
| `RESOURCE_HOLDER` | conversation holding an external scarce resource (Teradata session handle, MCP slot) | priority inheritance: it is blocking other agents who are burning LLM budget trying to acquire what it holds |
