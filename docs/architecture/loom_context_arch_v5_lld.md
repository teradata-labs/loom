...
1. **`manage_skills` / `manage_patterns`** — `list`/`load` (+ `unload`) wrappers over the existing narrowing, high-risk gate (now an explicit tool result, not a silent log-skip), `RegisterTools`, task emission, and the library loader. The load result is the charter event (class set per Part A) and includes the skill's folder path (remote-exec seam). Registered via `RegisterLazyTools` (`agent.go:771-809`).
...
3. **Active set: no implicit eviction.** The score-based eviction (`orchestrator.go:443-449`, `MaxConcurrentSkills=3`) is deleted. The cap was sized for injected-body cost, which no longer exists; silent eviction drops a skill's tool wiring (`excluded_tools`, required tools) mid-workflow while its body remains in the conversation. Cap becomes a safety limit (default 20) returning an explicit `manage_skills` error; removal only via explicit `unload` or session end. The active set keeps driving `applySkillExcludedTools` / `enforceRequiredSkillTools` / task emission, written at load time.
