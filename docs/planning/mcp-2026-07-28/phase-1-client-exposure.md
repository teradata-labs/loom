...
- **[proposed] Transport surfaces HTTP status as a typed error.** `handleHTTPStatus` (`pkg/mcp/transport/streamable_http.go:344`) wraps non-2xx as `*transport.HTTPStatusError{Code int}`. `Connect` falls back to `Initialize` on `errors.As` codes 404, 405, 501 in addition to JSON-RPC `MethodNotFound`. 401/403/5xx stay hard errors — auth failures and outages must not be masked as "legacy server". **[amended per PR #327 review rounds 1–2]** The fallback set as implemented is wider than this original decision: any non-modern JSON-RPC error (the stdio rule forbids keying to one code — legacy servers commonly answer `-32601` or `-32602`), HTTP 400 without a modern error body (strict legacy session servers), and — stdio only — probe silence past the bounded `RequestTimeout` window. Recognized modern errors (`-32020`/`-32021`/`-32022`) and HTTP probe timeouts never fall back.
...
2. **`x-mcp-header` parameter pass-through.** Implement the SEP-2243 mechanism exactly as the spec defines the schema flag (extract the flag definition from the spec — the parent doc deliberately doesn't restate it). Loom's own bridge tools don't use header parameters, so this is client-side conformance only. **[corrected per PR #327 review finding 12]** The actual contract: the argument value **stays in the body** and is *mirrored* into an HTTP header named `Mcp-Param-{name}` (where `{name}` is the `x-mcp-header` annotation value), Base64-sentinel-encoded when not header-safe; servers validate that header and body match (`HeaderMismatch` on divergence). Nothing is ever removed from the body, and no `x-mcp-header-*` header exists on the wire. Enforcement (invalid-annotation rejection and mirroring) applies only to stateless Streamable HTTP connections; other transports ignore the annotations.
3. **Manager switch.** `pkg/mcp/manager/manager.go:170`: `Initialize` → `Connect`. Add a getter on the client for the negotiated revision (field `c.protocolVersion` exists; expose `NegotiatedVersion() string`) and log it per server at Info. **[amended during implementation]** The inventory missed three more Loom-as-client paths that call `Initialize` directly — `pkg/docker/mcp_manager.go`, `pkg/backends/teradata/backend.go`, and `pkg/fabric/factory/factory.go` — plus `InstrumentedClient`, which had no `Connect` wrapper. All switched to `Connect`; `InstrumentedClient` gained instrumented `Connect`/`NegotiatedVersion`/`IsStateless` (span `mcp.client.connect`).
...
| 14 | `tools/call` with a header-flagged parameter | Send | Value mirrored as `Mcp-Param-{name}` header AND retained in the body arguments (servers reject header/body mismatch) |
