...
- [x] Request chunking + bisection (`decision.Chunk`, `DecisionRequest.fan_out_key`, `DecisionConfig.max_questions_per_request`, Jev hint 16) — 2026-09-24, after the A/B showed gateway 503s scale with request size (0% ≤10 questions → 39% at 51–64). Wired at every router construction. See architecture "Large requests".
- [x] Live budget scales with the chunk plan (`decision.Waves`/`decision.Budget`, 4 s per round capped at 12 s) and the Jev client stops retrying when the caller's deadline cannot hold the backoff — 2026-09-24, after cell B lost 24% of its 49-64-candidate visits to the old flat 3 s cap while the same requests run uncapped finished inside 4 s 91% of the time.
- [x] LongMemEval A/B for 3.1 (A = LLM rerank + Jev shadow, B = Jev live on `recall.rerank`, N=40 multi-session + knowledge-update, gpt-4o server, per-decision grading against evidence sessions via `grade_recall.py`). Relaunched 2026-09-23 19:55Z after finding that isolate-mode temp agents were named by question id alone, so a rerun recalled memories left by earlier runs (`DeleteAgent` does not purge graph memory); fixed with a per-run nonce in the agent name and session provenance on `graph_memory`-tool writes. Second fix 2026-09-24 00:19Z: `decision.SessionIDFromContext` ignored the agent session, so every live-turn row had an empty `session_id`. **Cell A multi-session readout (1,061 graded decisions):** Jev precision 95.0% / recall 56.0% vs LLM rerank 97.9% / 40.4%; end-to-end 20/37 (54.1%). Cell B and the knowledge-update cells still running. **DONE 2026-09-24**, all four cells: A/multi 20/37, B/multi 22/37, A/ku 25/39, B/ku 17/39; no difference significant (pooled p = 0.33). Jev is mid-90s precision at 50-57% recall in every cell. A threshold sweep over 1,912 graded candidates says `act_min` 0.2-0.3, not the 0.5 the A/B ran (recall 74.6% vs 54.6% for 4.5 points of precision). Full write-up: report §8.
...
- [x] 4.4 skill selection — `skill.route` site + live/shadow paths in `skills/index.Router`, both generative calls in the walk (per-node `askDecision` and the fat-leaf pick). Corpus test: 32 skills over 8 domains, 34 labelled messages (literal / paraphrase / distractor), a decider that reads only what the request carries. Found and fixed en route: `Route` collected selected skills in a map and then capped, so identical input could return different skills. **Measured against real Jev and the answer is no: best of ten band settings is 16/73 (22%) end to end, 33 wrong-domain loads, 42/73 empty; per decision, precision 50.9% at recall 96.7% over 350 labelled judgements. Stays in shadow.**
- [x] 5.x extraction trigger — `memory.extract`, one Noul in front of `extractGraphMemoryAsync`, skip-only. Reference is observed yield (did the run store anything), so shadow mode is a labelled dataset rather than an agreement measurement. Tests cover the polarity table, true_min narrowing skips, no-answer, window rendering dropping tool payloads, and the agent paths (skip / extract / shadow / layer off).
