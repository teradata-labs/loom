...
- [ ] Phase 3 evidence: run the six sites in shadow with `provider: jev` on the rig (graph memory on, tool_search BALANCED, a conditional workflow in the mix), produce `loom decision report --site` for each, then pick bands. Recall precision on the LongMemEval harness before/after is the 3.1 acceptance gate.
