...
// SuggestedTaxonomies seeds the classifier prompt with known
// parent_index_path buckets per domain. The classifier may propose new
// sibling paths under the same root when none of the suggested buckets
// fit, but it must not invent unrelated top-level roots — that keeps
// the resulting tree shallow.
//
...
// skill from the suggested taxonomy for its domain. The LLM may propose
// a new sibling under the same domain root if none of the suggestions
// fit; the validator rejects hallucinated unrelated top-level roots.
//
// Returns the chosen path, or an empty string when classification fails.
// Empty means "use the legacy unclassified/<domain> placement" computed
// by SkillPath() at index-build time. Errors are surfaced to the caller
// for logging; the caller is expected to treat them as soft failures.
//
...
// the catalog. The graph-aware variant (ClassifyAgainstGraph, future
// work) extends this with per-bucket member lists from the persisted
// router index.
func Classify(ctx context.Context, llm types.LLMProvider, imp *Skill, domain string) (string, error) {
	if llm == nil {
...
func buildClassifyPrompt(imp *Skill, domain string, suggestions []string) string {
...
// truncateClassify caps a string at n bytes; mirrored locally to keep
