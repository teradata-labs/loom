...
func TestValidateToolSchemas_DetectsMissingType(t *testing.T) {
	tools := []openai.Tool{
		{
			Type: "function",
			Function: openai.FunctionDef{
	...
	errors := ValidateToolSchemas(tools)
	assert.NotEmpty(t, errors, "should detect missing type")

	foundTypeError := false
	for _, err := range errors {
		if contains(err, "missing 'type' field") {
			foundTypeError = true
			break
		}
	}
	assert.True(t, foundTypeError, "should report missing type field")
}

func TestValidateToolSchemas_DetectsMissingProperties(t *testing.T) {
	tools := []openai.Tool{
		{
			Type: "function",
			Function: openai.FunctionDef{
	...
	errors := ValidateToolSchemas(tools)
	assert.NotEmpty(t, errors, "should detect missing properties")

	foundPropsError := false
	for _, err := range errors {
		if contains(err, "missing 'properties' field") {
			foundPropsError = true
			break
		}
	}
	assert.True(t, foundPropsError, "should report missing properties field")
}
