...
func runLearningAnalyze(cmd *cobra.Command, args []string) {
	...
	// Call analyze
	fmt.Printf("🔍 Analyzing pattern effectiveness...\n")
	if learningDomain != "" {
		fmt.Printf("   Domain: %s\n", learningDomain)
	}
	if learningAgent != "" {
	...
	fmt.Printf("📊 Pattern Analysis (%d patterns analyzed)\n", len(resp.Patterns))
	...
	fmt.Println(strings.Repeat("─", 80))
	fmt.Printf("\n✅ Analysis complete. Use 'looms learning proposals' to see improvement suggestions.\n")
}
...
func runLearningProposals(cmd *cobra.Command, args []string) {
	...
	fmt.Printf("📋 Listing improvement proposals...\n")
	if learningDomain != "" {
		fmt.Printf("   Domain: %s\n", learningDomain)
	}
	if learningAgent != "" {
	...
	// Display proposals
	fmt.Printf("💡 Improvement Proposals (%d found)\n", len(filtered))
	fmt.Println(strings.Repeat("─", 80))
	...
	for i, improvement := range filtered {
		...
		if improvement.Status == loomv1.ImprovementStatus_IMPROVEMENT_PENDING {
			fmt.Printf("    💡 Apply: looms learning apply %s\n", improvement.Id)
		}
	}
	...
	if pendingCount > 0 {
		fmt.Printf("\n✨ %d pending improvements ready to apply.\n", pendingCount)
	}
}
...
func runLearningApply(cmd *cobra.Command, args []string) {
	...
	// Apply improvement
	fmt.Printf("🚀 Applying improvement %s...\n", improvementID)
	...
	fmt.Printf("\n💡 Pattern hot-reloaded. Changes are now active.\n")
...
func runLearningRollback(cmd *cobra.Command, args []string) {
	...
	fmt.Printf("⏪ Rolling back improvement %s...\n", improvementID)
	...
	// Display result
	fmt.Printf("\n✅ Improvement rolled back successfully!\n\n")
	fmt.Printf("   ID: %s\n", improvementID)
	if resp.Success {
	...
	fmt.Printf("\n💡 Pattern hot-reloaded. Previous version restored.\n")
}
...
func runLearningHistory(cmd *cobra.Command, args []string) {
	...
	// Get history
	fmt.Printf("📜 Improvement History\n")
	if learningDomain != "" {
		fmt.Printf("   Domain: %s\n", learningDomain)
	}
	if learningAgent != "" {
...
func runLearningStream(cmd *cobra.Command, args []string) {
	...
	// Print header
	fmt.Printf("📡 Streaming pattern metrics from %s\n", learningServer)
	if learningDomain != "" {
...
func runLearningTune(cmd *cobra.Command, args []string) {
	...
	// Print configuration
	fmt.Printf("🎯 Pattern Tuning Configuration\n")
	fmt.Println(strings.Repeat("─", 80))
	if learningDomain != "" {
		fmt.Printf("   Domain: %s\n", learningDomain)
	}
	...
	if useDimensionWeights {
		...
		fmt.Printf("\n   💡 Patterns with judge scores will use dimension-weighted scoring\n")
		fmt.Printf("      Patterns without judge scores will fall back to legacy weights\n")
	} else {
		fmt.Printf("\n   ⚠️  Using Legacy Weights (consider using --dimension-* flags):\n")
		fmt.Printf("   - Quality: %.1f%%\n", learningTuneQualWeight*100)
		fmt.Printf("   - Cost:    %.1f%%\n", learningTuneCostWeight*100)
		fmt.Printf("   - Latency: %.1f%%\n", learningTuneLatWeight*100)
		fmt.Printf("\n   💡 Tip: Use --dimension-quality, --dimension-safety, etc. for judge-based tuning\n")
	}

	fmt.Println(strings.Repeat("─", 80))
	fmt.Printf("\n🔍 Analyzing patterns and calculating tunings...\n\n")
	...
	fmt.Printf("📊 Pattern Tuning Results (%d patterns analyzed)\n", resp.Summary.PatternsAnalyzed)
	fmt.Println(strings.Repeat("─", 80))
	...
	fmt.Println()
	if learningTuneDryRun {
		fmt.Println("💡 This was a dry run. To apply changes, run without --dry-run flag.")
	} else {
		fmt.Println("✅ Pattern tuning applied! Changes are now active.")
