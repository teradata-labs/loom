...
//
// Status per value is stated on the value. Only the ✅ triggers are fired by the
// runtime today; the 📋 ones parse, validate and resolve, but no code path
// passes them to the emitter yet, so selecting only those records nothing (the
// agent logs a warning when the resolved set cannot fire).
type ImplicitTaskTrigger int32

const (
	// Not a trigger. Load-bearing for fail-closed config parsing: an
	// unrecognised trigger name resolves to this and disables emission, and the
	// policy resolver drops it from any trigger set.
	ImplicitTaskTrigger_IMPLICIT_TASK_TRIGGER_UNSPECIFIED ImplicitTaskTrigger = 0
	// ✅ Fired: the agent invoked a tool.
	ImplicitTaskTrigger_IMPLICIT_TASK_TRIGGER_TOOL_CALL ImplicitTaskTrigger = 1
	// 📋 Not yet fired by the runtime: a skill activated for this turn. The
	// skills task emitter already creates tasks on activation, which is why this
	// is outside the default set.
	ImplicitTaskTrigger_IMPLICIT_TASK_TRIGGER_SKILL_ACTIVATION ImplicitTaskTrigger = 2
	// ✅ Fired: the agent asked a human for approval, a decision, input, or
	// review.
	ImplicitTaskTrigger_IMPLICIT_TASK_TRIGGER_HUMAN_REQUEST ImplicitTaskTrigger = 3
	// 📋 Not yet fired by the runtime: the agent spawned an ephemeral subagent.
	ImplicitTaskTrigger_IMPLICIT_TASK_TRIGGER_SUBAGENT_SPAWN ImplicitTaskTrigger = 4
	// 📋 Not yet fired by the runtime: a workflow stage began executing. The
	// task-tracked orchestrator already creates a task per stage, which is why
	// this is outside the default set.
...
type AgentConfig struct {
	...
	// Decision layer configuration (pkg/decision): which decider answers typed
	// auxiliary judgments and, per call site, whether its answers are acted on
	// or only shadow-recorded. Absent or provider "off" disables the layer.
	Decision      *DecisionConfig `protobuf:"bytes,21,opt,name=decision,proto3" json:"decision,omitempty"`
...
func (x *AgentConfig) GetDecision() *DecisionConfig {
	if x != nil {
		return x.Decision
	}
	return nil
...
type MemoryConfig struct {
	...
	// Graph memory configuration for salience-driven episodic memory.
	GraphMemory *GraphMemoryConfig `protobuf:"bytes,8,opt,name=graph_memory,json=graphMemory,proto3" json:"graph_memory,omitempty"`
	// Task board configuration for dependency-aware task decomposition and kanban.
	TaskBoard     *TaskBoardConfig `protobuf:"bytes,9,opt,name=task_board,json=taskBoard,proto3" json:"task_board,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *MemoryConfig) GetGraphMemory() *GraphMemoryConfig {
	if x != nil {
		return x.GraphMemory
	}
	return nil
}

func (x *MemoryConfig) GetTaskBoard() *TaskBoardConfig {
	if x != nil {
		return x.TaskBoard
	}
	return nil
...
// GraphMemoryConfig configures the salience-driven graph-backed episodic memory layer.
type GraphMemoryConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Whether graph memory is enabled.
	Enabled bool `protobuf:"varint,1,opt,name=enabled,proto3" json:"enabled,omitempty"`
	// Memory budget as percentage of max context window (0-100).
	// Default: 10 (10% of context). For 200K context -> 20K tokens.
	// Set to 0 to use max_context_tokens instead.
	ContextBudgetPercent int32 `protobuf:"varint,2,opt,name=context_budget_percent,json=contextBudgetPercent,proto3" json:"context_budget_percent,omitempty"`
	// Fixed token budget (overrides context_budget_percent if > 0).
	// Default: 0 (use percentage-based budget).
	...
	// Salience decay rate per day. Default: 0.995.
	// 0.995^138 ~ 0.5 (half-life ~138 days).
	DecayRate float32 `protobuf:"fixed32,4,opt,name=decay_rate,json=decayRate,proto3" json:"decay_rate,omitempty"`
	// Salience boost per access. Default: 0.05.
	BoostAmount float32 `protobuf:"fixed32,5,opt,name=boost_amount,json=boostAmount,proto3" json:"boost_amount,omitempty"`
	// Minimum salience for recall. Default: 0.1.
	MinSalienceThreshold float32 `protobuf:"fixed32,6,opt,name=min_salience_threshold,json=minSalienceThreshold,proto3" json:"min_salience_threshold,omitempty"`
	// Max memories to consider during recall. Default: 50.
...
func (x *GraphMemoryConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GraphMemoryConfig) ProtoMessage() {}

func (x *GraphMemoryConfig) ProtoReflect() protoreflect.Message {
	mi := &file_loom_v1_agent_config_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use GraphMemoryConfig.ProtoReflect.Descriptor instead.
func (*GraphMemoryConfig) Descriptor() ([]byte, []int) {
	return file_loom_v1_agent_config_proto_rawDescGZIP(), []int{9}
}

func (x *GraphMemoryConfig) GetEnabled() bool {
	if x != nil {
		return x.Enabled
	}
	return false
...
func (x *GraphMemoryConfig) GetContextBudgetPercent() int32 {
	if x != nil {
...
func (x *GraphMemoryConfig) GetMaxContextTokens() int32 {
	if x != nil {
		return x.MaxContextTokens
	}
	return 0
...
func (x *GraphMemoryConfig) GetDecayRate() float32 {
	if x != nil {
		return x.DecayRate
	}
	return 0
...
func (x *GraphMemoryConfig) GetBoostAmount() float32 {
	if x != nil {
...
func (x *GraphMemoryConfig) GetMinSalienceThreshold() float32 {
	if x != nil {
		return x.MinSalienceThreshold
	}
	return 0
}

func (x *GraphMemoryConfig) GetMaxRecallCandidates() int32 {
	if x != nil {
		return x.MaxRecallCandidates
	}
	return 0
}
...
type TaskBoardConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Whether the task board is enabled. Default: false (opt-in).
	Enabled bool `protobuf:"varint,1,opt,name=enabled,proto3" json:"enabled,omitempty"`
	// Automatically decompose complex goals into task DAGs.
	AutoDecompose bool `protobuf:"varint,2,opt,name=auto_decompose,json=autoDecompose,proto3" json:"auto_decompose,omitempty"`
	...
	// Default board ID. If empty, the agent creates/uses a board named after itself.
	DefaultBoardId string `protobuf:"bytes,4,opt,name=default_board_id,json=defaultBoardId,proto3" json:"default_board_id,omitempty"`
	// Default decomposition strategy.
	DefaultStrategy DecomposeStrategy `protobuf:"varint,5,opt,name=default_strategy,json=defaultStrategy,proto3,enum=loom.v1.DecomposeStrategy" json:"default_strategy,omitempty"`
...
func (x *TaskBoardConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TaskBoardConfig) ProtoMessage() {}

func (x *TaskBoardConfig) ProtoReflect() protoreflect.Message {
	mi := &file_loom_v1_agent_config_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use TaskBoardConfig.ProtoReflect.Descriptor instead.
func (*TaskBoardConfig) Descriptor() ([]byte, []int) {
	return file_loom_v1_agent_config_proto_rawDescGZIP(), []int{10}
}

func (x *TaskBoardConfig) GetEnabled() bool {
	if x != nil {
		return x.Enabled
	}
	return false
...
func (x *TaskBoardConfig) GetAutoDecompose() bool {
	if x != nil {
		return x.AutoDecompose
	}
	return false
}

func (x *TaskBoardConfig) GetMaxDepth() int32 {
	if x != nil {
		return x.MaxDepth
	}
	return 0
...
func (x *TaskBoardConfig) GetDefaultBoardId() string {
	if x != nil {
		return x.DefaultBoardId
	}
	return ""
}

func (x *TaskBoardConfig) GetDefaultStrategy() DecomposeStrategy {
	if x != nil {
...
func (x *TaskBoardConfig) GetContextBudgetTokens() int32 {
	if x != nil {
...
const file_loom_v1_agent_config_proto_rawDesc = "" +
	"\n" +
	"\x1aloom/v1/agent_config.proto\x12\aloom.v1\x1a\x16loom/v1/decision.proto\x1a\x13loom/v1/skill.proto\x1a\x12loom/v1/task.proto\"\xa5\b\n" +
	...
	"\x06skills\x18\x14 \x01(\v2\x15.loom.v1.SkillsConfigR\x06skills\x123\n" +
	"\bdecision\x18\x15 \x01(\v2\x17.loom.v1.DecisionConfigR\bdecision\x1a;\n" +
	...
	"\x04name\x18\x01 \x01(\tR\x04name\x12&\n" +
	"\x0eimplementation\x18\x02 \x01(\tR\x0eimplementation\"\x9f\x03\n" +
	...
	"\x12max_context_tokens\x18\x03 \x01(\x05R\x10maxContextTokens\x12\x1d\n" +
	"\n" +
	"decay_rate\x18\x04 \x01(\x02R\tdecayRate\x12!\n" +
...
var file_loom_v1_agent_config_proto_goTypes = []any{
	...
	(*DecisionConfig)(nil),              // 29: loom.v1.DecisionConfig
	(DecomposeStrategy)(0),              // 30: loom.v1.DecomposeStrategy
}
var file_loom_v1_agent_config_proto_depIdxs = []int32{
	...
	29, // 11: loom.v1.AgentConfig.decision:type_name -> loom.v1.DecisionConfig
	8,  // 12: loom.v1.ProviderEntry.config:type_name -> loom.v1.LLMConfig
	6,  // 13: loom.v1.ProviderPool.providers:type_name -> loom.v1.ProviderEntry
	9,  // 14: loom.v1.LLMConfig.rate_limit:type_name -> loom.v1.LLMRateLimitConfig
	11, // 15: loom.v1.ToolsConfig.mcp:type_name -> loom.v1.MCPToolConfig
	12, // 16: loom.v1.ToolsConfig.custom:type_name -> loom.v1.CustomToolConfig
	18, // 17: loom.v1.MemoryConfig.memory_compression:type_name -> loom.v1.MemoryCompressionConfig
	14, // 18: loom.v1.MemoryConfig.graph_memory:type_name -> loom.v1.GraphMemoryConfig
	15, // 19: loom.v1.MemoryConfig.task_board:type_name -> loom.v1.TaskBoardConfig
	30, // 20: loom.v1.TaskBoardConfig.default_strategy:type_name -> loom.v1.DecomposeStrategy
	16, // 21: loom.v1.TaskBoardConfig.implicit_tasks:type_name -> loom.v1.ImplicitTaskConfig
	1,  // 22: loom.v1.ImplicitTaskConfig.mode:type_name -> loom.v1.ImplicitTaskMode
	2,  // 23: loom.v1.ImplicitTaskConfig.triggers:type_name -> loom.v1.ImplicitTaskTrigger
	2,  // 24: loom.v1.ImplicitTaskConfig.excluded_triggers:type_name -> loom.v1.ImplicitTaskTrigger
	3,  // 25: loom.v1.MemoryCompressionConfig.workload_profile:type_name -> loom.v1.WorkloadProfile
	17, // 26: loom.v1.MemoryCompressionConfig.batch_sizes:type_name -> loom.v1.MemoryCompressionBatchSizes
	20, // 27: loom.v1.BehaviorConfig.patterns:type_name -> loom.v1.PatternConfig
	22, // 28: loom.v1.AgentTemplate.parameters:type_name -> loom.v1.TemplateParameter
	5,  // 29: loom.v1.AgentTemplate.template_config:type_name -> loom.v1.AgentConfig
	5,  // 30: loom.v1.AgentProfile.defaults:type_name -> loom.v1.AgentConfig
	27, // 31: loom.v1.AgentProfile.overrides:type_name -> loom.v1.AgentProfile.OverridesEntry
	25, // 32: loom.v1.EphemeralAgentPolicy.trigger:type_name -> loom.v1.SpawnTrigger
	5,  // 33: loom.v1.EphemeralAgentPolicy.template:type_name -> loom.v1.AgentConfig
	4,  // 34: loom.v1.SpawnTrigger.type:type_name -> loom.v1.SpawnTriggerType
	5,  // 35: loom.v1.AgentProfile.OverridesEntry.value:type_name -> loom.v1.AgentConfig
