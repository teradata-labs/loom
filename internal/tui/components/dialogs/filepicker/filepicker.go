...
import (
	...
	"github.com/teradata-labs/loom/internal/tui/components/image"
	"github.com/teradata-labs/loom/internal/tui/styles"
	"github.com/teradata-labs/loom/internal/tui/util"
)

const (
	MaxAttachmentSize   = int64(50 * 1024 * 1024) // 50MB (server allows 100MB)
...
// AllowedTypes is empty to accept all file types (not just images)
...
func NewFilePickerCmp(workingDir string) FilePicker {
	...
	if workingDir != "" {
		...
		if cwd, err := os.Getwd(); err == nil {
			...
			fp.CurrentDirectory = home.UserHome()
		}
	}
...
func (m *model) Update(msg tea.Msg) (util.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		if key.Matches(msg, m.filePicker.KeyMap.Back) {
			...
			if m.filePicker.CurrentDirectory == home.UserHome() {
				return m, nil
			}
		}
	}
...
func (m *model) View() string {
	...
	strs := []string{
		t.S().Base.Padding(0, 1, 1, 1).Render(core.Title("Add File", m.width-4)),
	}
