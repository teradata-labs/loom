...
import (
	...
	"github.com/teradata-labs/loom/internal/agent/tools"
	"github.com/teradata-labs/loom/internal/ansiext"
	"github.com/teradata-labs/loom/internal/fsext"
	"github.com/teradata-labs/loom/internal/tree"
	...
	"github.com/teradata-labs/loom/internal/tui/highlight"
	"github.com/teradata-labs/loom/internal/tui/styles"
)

// responseContextHeight limits the number of lines displayed in tool output
// Set to a high value to show full content without truncation
const responseContextHeight = 500
	...
	if len(v.result.Data) > 0 {
		if strings.HasPrefix(v.result.MIMEType, "image/") {
			return br.renderWithParams(v, v.call.Name, nil, func() string {
				return renderImageContent(v, string(v.result.Data), v.result.MIMEType, v.result.Content)
			})
		}
		return br.renderWithParams(v, v.call.Name, nil, func() string {
...
func (vr viewRenderer) Render(v *toolCallCmp) string {
	...
	return vr.renderWithParams(v, "View", args, func() string {
		if len(v.result.Data) > 0 && strings.HasPrefix(v.result.MIMEType, "image/") {
			return renderImageContent(v, string(v.result.Data), v.result.MIMEType, "")
		}
...
func (mer multiEditRenderer) Render(v *toolCallCmp) string {
	...
	return mer.renderWithParams(v, "Multi-Edit", args, func() string {
		...
		if meta.EditsFailed > 0 {
			noteTag := t.S().Base.Padding(0, 2).Background(t.Info).Foreground(t.White).Render("Note")
			noteMsg := fmt.Sprintf("%d of %d edits succeeded", meta.EditsApplied, meta.EditsApplied+meta.EditsFailed)
			note := t.S().Base.
				Width(v.textWidth() - 2).
