...
func (m *messageCmp) Update(msg tea.Msg) (util.Model, tea.Cmd) {
	...
	case anim.StepMsg:
		m.spinning = m.shouldSpin()
		// Update animation for both spinning (no content) and thinking (progress with reasoning content)
		if m.spinning || (m.message.IsThinking() && m.anim.Running()) {
			m.anim.Tick()
			return m, nil
		}
...
func (m *messageCmp) SetMessage(msg message.Message) {
	m.message = msg
	// Update spinning state based on new message content
	m.spinning = m.shouldSpin()
}
...
func (m *messageCmp) renderThinkingContent() string {
	...
	// Start animation if we're still thinking (not finished)
	if reasoningContent.StartedAt > 0 && reasoningContent.FinishedAt == 0 && !m.anim.Running() {
		m.anim.Start()
	}
	...
	// Allow up to 100 lines of thinking/progress content (was 10)
	height := ordered.Clamp(lipgloss.Height(fullContent), 1, 100)
